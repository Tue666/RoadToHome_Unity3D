...
public class SwitchCrosshair : MonoBehaviour
{
    ...
    private Transform cameraFOV = null;

    // Start is called before the first frame update
    void Start()
    {
        cameraFOV = PlayerManager.Instance.cameraRecoil.transform;
        if (crosshair == null)
        {
    ...
    IEnumerator SwitchOnFocusedTarget()
    {
        ...
        {
            RaycastHit hit;
            if (Physics.Raycast(cameraFOV.position, cameraFOV.forward, out hit))
            {
